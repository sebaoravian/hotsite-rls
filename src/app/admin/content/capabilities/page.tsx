...
const iconOptions: IconName[] = ['cloud', 'mobile', 'web', 'code', 'chart', 'shield', 'bulb', 'clipboard', 'globe']
...
export default function CapabilitiesPage() {
  ...
  const fetchData = async () => {
    try {
      const res = await fetch('/api/content/capabilities/items')
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        const res = await fetch(`/api/content/capabilities/items/${editingId}`, {
          method: 'PUT',
  ...
  const handleDelete = async (id: string) => {
    if (!confirm('¿Seguro que quieres eliminar esta capability?')) return

    try {
      const res = await fetch(`/api/content/capabilities/items/${id}`, {
        method: 'DELETE'
      })

      if (res.ok) {
        alert('✅ Capability eliminada')
        fetchData()
      }
    } catch (error) {
    ...
    <div>
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600">Gestiona las capacidades técnicas del equipo</p>
        </div>
        <button
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-xl p-6 shadow-sm border border-gray-200 mb-6">
        <h2 className="text-lg font-bold mb-4">
          {editingId ? 'Editar Capability' : 'Nueva Capability'}
        </h2>
      ...
      <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-200">
        <h2 className="text-lg font-bold mb-4">Capabilities Actuales ({items.length})</h2>
        
        {items.length === 0 ? (
          <p className="text-gray-500 text-center py-8">No hay capabilities todavía</p>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {items.map((item) => (
              ...
              >
                ...
                <p className="text-sm text-gray-600" style={{ color: item.color }}>
                  {item.description}
                </p>
              </div>
            ))}
